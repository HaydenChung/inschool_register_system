...
import { withUiSound } from "../../context";
...
class RegisterPage extends React.Component {
    ...
    toggleFreePassPeriod({ expire = 30, state = true, approver }) {
        ...
        const newState = {
            freePassPeriod: true,
            approver:
                typeof approver == "undefined" ? this.state.approver : approver
        };

        this.setState(newState);
        this.freePassTimer = setTimeout(
            () => this.setState({ freePassPeriod: false, approver: "" }),
            expire * 1000
        );
    }
        ...
        if (ev.keyCode === 13 || ev.key === "Enter") {
            ...
            this.fetchUser(cardId)
                .then(response => {
                    if (!response)
                        throw new Error(
                            "No user found, please check the Smartcard id."
                        );
                    ...
                    if (student["role_id"] == 4) {
                        this.toggleFreePassPeriod({
                            state: true,
                            approver: student["uid"]
                        });
                        freePass = true;
                    }

                    if (freePass) {
                        return this.updateUserInschool(cardId).then(
                            response => {
                                if (response >= 0) {
                                    this.toggleFreePassPeriod({ state: true });
                                    this.setState({
                        ...
                        return this.checkWhiteList(cardId).then(response => {
                            if (
                                typeof response[0] != "undefined" &&
                                typeof response[0].uid != "undefined"
                            ) {
                                return this.updateUserInschool(cardId).then(
                                    response => {
                                        this.setState({
                                ...
                                ajaxCall({
                                    ...
                                    this.setState(newState);
                                    if (newState.studentData.isVerify == true) {
                                        this.props.uiSound.acceptSound.play();
                                    } else {
                                        this.props.uiSound.denialSound.play();
                                    }
                                });
                            }
                        });
    ...
    onRegisterFailHandler(error) {
        ...
        reminder(
            "Register Failed, please check your connection or contact support staff."
        );
                ...
                <Template active={this.state.active}>
                    ...
                    >
                        <HiddenInput
                            ...
                            onKeyPress={this.inputOnKeyPressHandler}
                        />
                        <LargeUserCard
                            freePassPeriod={this.state.freePassPeriod}
                            chi_name={student.chi_name}
                            eng_name={student.eng_name}
                            class={student.class}
                            class_number={student.class_number}
                            image={student.uri}
                            student_no={student.name}
...
export default withUiSound(RegisterPage);
